module SessionHelper
	# Sign in user by creating a new remember token, store it in cookie and encrpyted version on database
	def sign_in(user)
	...
	# calling this function within a view, or other controllers
	def signed_in?
		!self.current_user.nil?
	end

	# Sign user out by replacing user's remember token in dabatse with a random one and delete remember token in cookie
	def sign_out
		# Make a random remember token in case user cookie is every stolen
		randomRememberToken = User.new_remember_token
		current_user.update_attribute(:remember_token, User.encrypt(randomRememberToken))
		# Delete remember token from cookies and set user to nil
